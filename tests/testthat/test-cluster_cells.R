...
test_that("cluster_cells works", {
  ...
  expect_equal(length(cds@clusters[["UMAP"]]$cluster_result$optim_res$membership),
               nrow(colData(cds)))
  expect_equal(cds@clusters[["UMAP"]]$cluster_result$optim_res$membership[[1]], 5)
  ...
  expect_equal(length(cds@clusters[["UMAP"]]$cluster_result$optim_res$membership),
               nrow(colData(cds)))
  expect_equal(cds@clusters[["UMAP"]]$cluster_result$optim_res$membership[[1]], 13)
  ...
  expect_equal(length(cds@clusters[["UMAP"]]$cluster_result$optim_res$membership),
               nrow(colData(cds)))
  expect_equal(cds@clusters[["UMAP"]]$cluster_result$optim_res$membership[[1]], 6)
  expect_equal(length(unique(clusters(cds, reduction_method = "UMAP"))), 11)
  ...
  expect_equal(length(cds@clusters[["UMAP"]]$cluster_result$optim_res$membership),
               nrow(colData(cds)))
  expect_equal(cds@clusters[["UMAP"]]$cluster_result$optim_res$membership[[1]], 3)
  expect_equal(length(unique(clusters(cds, reduction_method = "UMAP"))), 11)
  ...
  expect_equal(length(cds@clusters[["tSNE"]]$cluster_result$optim_res$membership),
               nrow(colData(cds)))
  expect_equal(cds@clusters[["tSNE"]]$cluster_result$optim_res$membership[[1]], 4)
  expect_equal(length(unique(clusters(cds, reduction_method = "tSNE"))), 10)

  # non-standard opts
  cds <- cluster_cells(cds, reduction_method = "tSNE", k=22,
                       resolution = c(0.75, 0.3), weight = T, num_iter = 2,
                       partition_qval = .1, verbose = TRUE, random_seed=2)
  expect_is(cds@clusters[["tSNE"]], "list")
  expect_equal(length(cds@clusters[["tSNE"]]), 3)
  expect_equal(length(cds@clusters[["tSNE"]]$cluster_result$optim_res$membership),
               nrow(colData(cds)))
  expect_equal(cds@clusters[["tSNE"]]$cluster_result$optim_res$membership[[1]], 8)
  ...
  expect_equal(length(cds@clusters[["PCA"]]$cluster_result$optim_res$membership),
               nrow(colData(cds)))
  expect_equal(cds@clusters[["PCA"]]$cluster_result$optim_res$membership[[1]], 1)
  expect_equal(length(unique(clusters(cds, reduction_method = "PCA"))), 23)
  ...
  expect_equal(length(cds@clusters[["PCA"]]$cluster_result$optim_res$membership),
               nrow(colData(cds)))
  expect_equal(cds@clusters[["PCA"]]$cluster_result$optim_res$membership[[1]], 16)
  expect_equal(length(unique(clusters(cds, reduction_method = "PCA"))), 62)
  ...
  expect_equal(length(cds@clusters[["PCA"]]$cluster_result$optim_res$membership),
               nrow(colData(cds)))
  expect_equal(as.numeric(cds@clusters[["PCA"]]$cluster_result$optim_res$membership[1]), 1)
  expect_equal(length(unique(clusters(cds, reduction_method = "PCA"))), 25)
  ...
  expect_equal(length(cds@clusters[["LSI"]]$cluster_result$optim_res$membership),
               nrow(colData(cds)))
  expect_equal(cds@clusters[["LSI"]]$cluster_result$optim_res$membership[1], 6)
  expect_equal(length(unique(clusters(cds, reduction_method = "LSI"))), 23)
  ...
  expect_equal(length(cds@clusters[["LSI"]]$cluster_result$optim_res$membership),
               nrow(colData(cds)))
  expect_equal(cds@clusters[["LSI"]]$cluster_result$optim_res$membership[[1]], 47)
  expect_equal(length(unique(clusters(cds, reduction_method = "LSI"))), 66)
})
...
test_that("cluster_cells works", {
  cds <- cluster_cells(cds)
  expect_equal(sum(clusters(cds) == 9), 35)
  expect_equal(sum(clusters(cds) == 1), 62)
  expect_equal(as.character(clusters(cds)[1]), "8")
})
