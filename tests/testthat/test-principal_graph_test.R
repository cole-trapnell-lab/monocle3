context("test-principal_graph_test")
...
# test_that("test principal_graph_test error messages work", {
#   expect_error(cds <- principal_graph_test(cds),
...
#   #expect_error(cds <- principal_graph_test(cds, learn_graph_control = list(prune = FALSE)), "Unknown variable in learn_graph_control")
# })

test_that("test principal_graph_test returns Dex-dependent genes",{
  test_cds = cds
  pos_ctrl_gene = test_cds[rowData(cds)$gene_short_name == "che-1",]
  pr_test_res = principal_graph_test(pos_ctrl_gene)
  ...
  pr_test_res = principal_graph_test(neg_ctrl_gene)
  expect_equal(pr_test_res$status[1], "OK")
...
test_that("test principal_graph_test returns few genes under UMAP coordinate randomization",{
  ...
  pr_test_res = principal_graph_test(test_cds, neighbor_graph="principal_graph", k=50)
