...
test_cds = cds
pr_graph_test_res = principal_graph_test(test_cds[1:200,])
pr_deg_ids = subset(pr_graph_test_res, q_value < 0.05)$id
gene_cluster_df = monocle3:::cluster_genes(test_cds[pr_deg_ids,])

png("module_graph.png", res=600, width=4, height=4, units="in")
monocle3:::plot_cells(test_cds, genes=gene_cluster_df, show_backbone=FALSE, label_branch_points=FALSE, label_leaves=FALSE, label_roots=FALSE)
dev.off()
