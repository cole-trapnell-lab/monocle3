...
expect_pseudotime_valid <- function(cds) {
  pt <- pseudotime(cds)
  testthat::expect_equal(length(pt), nrow(colData(cds)))
  testthat::expect_true(all(is.finite(pt)))
  testthat::expect_equal(min(pt), 0)
  testthat::expect_gt(max(pt), 0)
}
