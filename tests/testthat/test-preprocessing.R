...
test_that("preprocessing stays the same", {
  ...
  expect_equal(ncol(reducedDims(cds)$normalized_data_projection), 179)
  expect_equal(nrow(reducedDims(cds)$normalized_data_projection), nrow(colData(cds)))
  expect_equal(reducedDims(cds)$normalized_data_projection[1,"ENSG00000228253.1"],
  ...
  expect_equal(ncol(reducedDims(cds)$normalized_data_projection), 20)
  expect_equal(nrow(reducedDims(cds)$normalized_data_projection), nrow(colData(cds)))
  expect_equal(reducedDims(cds)$normalized_data_projection[1,1], 9.283694, tol = 1e-5)
  ...
  expect_equal(ncol(reducedDims(cds)$normalized_data_projection), 20)
  expect_equal(nrow(reducedDims(cds)$normalized_data_projection), nrow(colData(cds)))
  expect_equal(reducedDims(cds)$normalized_data_projection[1,1], -24.30544, tol = 1e-5)
  ...
  expect_equal(ncol(reducedDims(cds)$normalized_data_projection), 20)
  expect_equal(nrow(reducedDims(cds)$normalized_data_projection), nrow(colData(cds)))
  expect_equal(reducedDims(cds)$normalized_data_projection[2,1], 2.274213, tol = 1e-5)
