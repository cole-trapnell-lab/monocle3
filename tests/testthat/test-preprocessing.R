...
test_that("preprocessing stays the same", {
  ...
  expect_equal(reducedDims(cds)$PCA[1,1], 2.4207391, tol = 1e-5)

  cds <- preprocess_cds(cds, method = "LSI", num_dim = 20)
  expect_equal(ncol(reducedDims(cds)$LSI), 20)
  expect_equal(nrow(reducedDims(cds)$LSI), nrow(colData(cds)))
  expect_equal(reducedDims(cds)$LSI[1,1], 11.112528, tol = 1e-5)

  cds <- preprocess_cds(cds, method = "PCA", norm_method = "size_only", num_dim = 20)
  expect_equal(ncol(reducedDims(cds)$PCA), 20)
  expect_equal(nrow(reducedDims(cds)$PCA), nrow(colData(cds)))
  expect_equal(reducedDims(cds)$PCA[1,1], 2.222207, tol = 1e-5)

  cds <- preprocess_cds(cds, method = "LSI", norm_method = "size_only", num_dim = 20)
  expect_equal(ncol(reducedDims(cds)$LSI), 20)
  expect_equal(nrow(reducedDims(cds)$LSI), nrow(colData(cds)))
  expect_equal(reducedDims(cds)$LSI[1,1], 9.283694, tol = 1e-5)
