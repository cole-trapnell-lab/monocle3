...
plot_genes_violin(cds_subset, grouping="cell_type2")

plot_percent_cells_positive(cds_subset, grouping="cell_type2")
...
col_vector_origin <- c("#db83da",
              ...
              "#9edae5")
col_vector <-
  col_vector_origin[1:length(unique(as.character(colData(MCA_cds)$Tissue)))]
names(col_vector) <- unique(as.character(colData(MCA_cds)$Tissue))
...
options(repr.plot.height = 8)
cluster_col_vector <-
  col_vector_origin[1:length(unique(as.character(colData(MCA_cds)$Cluster)))]
names(cluster_col_vector) <- unique(as.character(colData(MCA_cds)$Cluster))
plot_cells(MCA_cds,
           color_by = 'Cluster',
           cell_size = 0.1,
           show_group_id = T) +
...
```{r}
Cluster_tissue_stat <- table(colData(MCA_cds)[, c('Cluster', 'Tissue')])
...
cluster_col_vector <-
  col_vector_origin[1:length(
    unique(as.character(colData(MCA_cds_res_resolution)$Cluster)))]
names(cluster_col_vector) <-
  unique(as.character(colData(MCA_cds_res_resolution)$Cluster))

plot_cells(MCA_cds_res_resolution,
           color_by = 'Cluster',
           cell_size = 0.1,
           show_group_id = T) +
  theme(legend.text=element_text(size=6)) + #set the size of the text
...
```{r}
my_genes <- row.names(subset(rowData(cds),
          gene_short_name %in% c("Cdk1", "Zwint", "Znhit1")))
