...
message('BPCells irlba now')
  ...
  x_squash <- BPCells::write_matrix_dir(x, tempfile("mat"), compress=FALSE)
# x_squash <- BPCells::write_matrix_memory(x, compress=FALSE)

  stats <- BPCells::matrix_stats(matrix = x_squash, row_stats = 'none', col_stats = 'variance')
  center <- stats[['col_stats']]['mean',]
  scale <- sqrt(stats[['col_stats']]['variance',])

  # BPCells::linear_operator() is meant to reduce irlba run time. It appears
  # to have little or no effect.
  args <- list(A = linear_operator(x_squash), nv = n, center = center, scale = scale)
