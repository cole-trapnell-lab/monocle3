...
reduce_dimension <- function(cds,
  ...
  if(reduction_method == "PCA") {
    ...
    cat( 'checksum: reduce_dimensions: start: umap in (as.matrix(preprocess_mat)): ', digest::digest( as.matrix(preprocess_mat) ), '\n')
    ...
    cat( 'checksum: reduce_dimensions: end: umap out (umap_res): ', digest::digest( umap_res ), '\n' )
