...
extract_coefficient_helper = function(model, term_labels,
  ...
  if (class(model) %in% c("vglm", "vgam")) {
    ...
    log_eff_over_int = log2((model@family@linkinv(coef_mat[, 1] + coef_mat[1, 1]) + pseudo_expr) / rep(
      model@family@linkinv(coef_mat[1, 1]) + pseudo_expr,
      times = nrow(coef_mat)
    )
    )
...
coefficient_table <- function(model_tbl) {
  M_f = model_tbl %>%
    dplyr::mutate(terms = purrr::map(.f = extract_coefficient_helper, .x = model, term_labels)) %>% tidyr::unnest(terms)
  return(M_f)
}
