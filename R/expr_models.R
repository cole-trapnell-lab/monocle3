...
estimate_gene_dispersions <- function(cds, min_cells_detected, remove_outliers, verbose = FALSE)
{
  ...
  res <- parametric_dispersion_fit(disp_table, verbose)
  fit <- res[[1]]
  coefs <- res[[2]]
  #removeOutliers = TRUE
  if (remove_outliers){
    ...
    res <- parametric_dispersion_fit(disp_table[row.names(disp_table) %in% outliers == FALSE,], verbose)
    fit <- res[[1]]
    coefs <- res[[2]]
  }
...
dispersion_table <- function (disp_results, sort_on=NULL, decreasing=TRUE)
{
  if (is.null(disp_results)) {
    ...
    stop("Error: no dispersion model found. Please call estimate_gene_dispersions() before calling this function")
  }
