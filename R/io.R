...
save_transform_models <- function( cds, directory_path, comment="", verbose=TRUE) {
  ...
  for(method in names(cds@preprocess_aux)) {
    ...
    methods_preprocess[[method]][['rds_path']] <- paste0('ppc_', tolower(method), '_transform_model.rds')
    methods_preprocess[[method]][['nn_index_path']] <- paste0('ppc_', tolower(method), '_transform_model_nn.idx')
  ...
  for(method in names(methods_preprocess)) {
    if(file.exists(file.path(directory_path, methods_preprocess[[method]][['rds_path']])))
      file.remove(file.path(directory_path, methods_preprocess[[method]][['rds_path']]))
    if(file.exists(file.path(directory_path, methods_preprocess[[method]][['nn_index_path']])))
      file.remove(file.path(directory_path, methods_preprocess[[method]][['nn_index_path']]))
  }
  ...
  for(method in names(methods_preprocess)) {
    tryCatch(
      {
        saveRDS(cds@preprocess_aux[[method]], file=file.path(directory_path, methods_preprocess[[method]][['rds_path']]))
      },
      error = function(cnd) {
                     message('Error writing file \'', file.path(directory_path, methods_preprocess[[method]][['rds_path']]), '\': ', cnd, appendLF=appendLF)
                     return(NULL)
      },
      finally = {
        md5sum <- tools::md5sum(file.path(directory_path, methods_preprocess[[method]][['rds_path']]))
    ...
    if(methods_preprocess[[method]][['has_nn_index']]) {
      tryCatch(
        {
          save_annoy_index(cds@preprocess_aux[[method]][['nn_index']][['annoy_index']], file.path(directory_path, methods_preprocess[[method]][['nn_index_path']]))
        },
        error = function(cnd) {
                       message('Error writing file \'', file.path(directory_path, methods_preprocess[[method]][['nn_index_path']]), '\': ', cnd, appendLF=appendLF)
                       return(NULL)
        },
        finally = {
          md5sum <- tools::md5sum(file.path(directory_path, methods_preprocess[[method]][['nn_index_path']]))
          file_index[['files']] <- rbind(file_index[['files']],
                                         data.frame(cds_object = 'preprocess_aux',
                                                    method = method,
