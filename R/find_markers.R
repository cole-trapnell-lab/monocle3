...
top_markers <- function(cds,
  ...
  # Now compute a Jensen Shannon specificity score for each gene w.r.t each
  # group
  cluster_spec_mat <- specificity_matrix(cluster_agg_exprs, cores=cores)
  cluster_spec_table <-
    tibble::rownames_to_column(as.data.frame(cluster_spec_mat))
  ...
  # Now compute the expected max specificity as a function of how many cells
  # express a given gene. Genes that are expressed in few cells tend to have
  # very high specificity, so we want to control for this trend when ranking
  # genes by specificity later on
  spec_model_df <- spec_model_df %>%
    dplyr::mutate(quantile = dplyr::ntile(num_expressing, expression_bins))
  spec_summary <- spec_model_df %>% dplyr::group_by(quantile) %>%
    dplyr::summarize(log_spec_mean = mean(log(max_spec)),
                     log_spec_sd = sd(log(max_spec)))
  ...
  cluster_spec_table <- cluster_spec_table %>%
    dplyr::mutate(log_spec=log(specificity),
                  pval_excess_spec = pnorm(log(specificity), log_spec_mean,
                                           log_spec_sd, lower.tail=FALSE))
  ...
  if (is.null(reference_cells) == FALSE){
    if(is.numeric(reference_cells)){
      num_ref_cells_per_group <-
        reference_cells / length(unique(cell_group_df$cell_group))
  ...
  marker_test_res <-
    tryCatch({pbmcapply::pbmcmapply(test_marker_for_cell_group,
  ...
  marker_test_res <- dplyr::bind_cols(cluster_spec_table,
  ...
  marker_test_res <- marker_test_res %>%
    dplyr::select(rowname, cell_group, specificity, pseudo_R2, lrtest_p_value,
                  lrtest_q_value)
  marker_test_res <- marker_test_res %>%
  ...
  marker_test_res$marker_test_p_value <-
    unlist(marker_test_res$marker_test_p_value)
